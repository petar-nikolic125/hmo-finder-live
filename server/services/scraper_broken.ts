...
export class ScrapingService {
  ...
  // Get city aliases and variations for validation
  ...
  async scrapeProperties(params: SearchParams): Promise<Property[]> {
    return new Promise((resolve, reject) => {
      ...
      // Enhanced strategy: Use enhanced scraper for better results especially when real scraper returns few results
      const scriptName = 'enhanced_scraper.py'; // Prefer enhanced scraper for better diversity
      const enhancedArgs = [...args];
      enhancedArgs[0] = path.join(process.cwd(), 'server/scraper/', scriptName);
      
      console.log(`🚀 Using ${scriptName} for better property diversity`);
      ...
      pythonProcess.on('close', async (code) => {
        ...
        try {
          ...
          if (!scrapedProperties || scrapedProperties.length === 0) {
            console.log('No properties found during scraping. Trying enhanced scraper fallback...');
            
            // Fallback to enhanced scraper if real scraper returns nothing
            if (!enhancedArgs[0].includes('enhanced_scraper.py')) {
              console.log('🔄 Switching to enhanced scraper for fallback results');
              const fallbackArgs = [...enhancedArgs];
              fallbackArgs[0] = path.join(process.cwd(), 'server/scraper/enhanced_scraper.py');
              
              const fallbackProcess = spawn('python3', fallbackArgs, {
                stdio: ['pipe', 'pipe', 'pipe'],
                cwd: process.cwd()
              });
              
              let fallbackOutput = '';
              fallbackProcess.stdout.on('data', (data) => fallbackOutput += data.toString());
              fallbackProcess.on('close', (code) => {
                if (code === 0 && fallbackOutput.trim()) {
                  try {
                    const fallbackProps = JSON.parse(fallbackOutput);
                    if (fallbackProps && fallbackProps.length > 0) {
                      console.log(`✅ Enhanced scraper provided ${fallbackProps.length} fallback properties`);
                      // Process fallback properties with the same logic
                      console.log('📦 Processing enhanced scraper results...');
                      const processedProps = this.convertScrapedToProperties(fallbackProps, params);
                      resolve(processedProps);
                      return;
                    }
                  } catch (e) {
                    console.log('Fallback scraper parsing failed');
                  }
                }
                resolve([]);
              });
          ...
          if (properties.length > 0) {
            this.cacheResults(params, properties);
  ...
  private convertScrapedToProperties(scrapedProperties: any[], params: SearchParams): Property[] {
    ...
    const properties: Property[] = [];
          ...
          for (const prop of scrapedProperties) {
            ...
            // Check if address contains the searched city or known variations
            const cityVariations = [
              searchCity,
              searchCity.substring(0, 4), // First 4 letters
